defmodule ClawEngine.OpenClaw.ChatClient do
  @moduledoc """
  ...
  def start_stream(opts) do
    ...
    with true <- gateway.token_present? || {:error, :missing_gateway_token} do
      state = %{
        ...
        token: gateway.token,
        version: Keyword.get(opts, :version, ClawEngine.version()),
        request_id: nil,
        connect_id: nil,
        connect_sent?: false,
        completed?: false,
  ...
  defp handle_incoming_frame(
    ...
    device_params = build_device_params(state, nonce, "gateway-client", "backend", scopes)
  ...
  # Fallback when no payload in connect response
  defp handle_incoming_frame(
